...
export function WalletSheet({ kind, total, itemsLabel, address, onDone, onClose }: { kind: Wallet; total: number; itemsLabel: string; address?: string; onDone: () => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[80]" role="dialog" aria-modal="true" aria-label={brand.name}>
      <button type="button" className="absolute inset-0 bg-black/55 backdrop-blur-sm" aria-label="Κλείσιμο" onClick={() => stage === "review" && onClose()} />
      <div className={`absolute inset-x-0 bottom-0 @md:inset-auto @md:left-1/2 @md:top-1/2 @md:-translate-x-1/2 @md:-translate-y-1/2 @md:w-[420px] rounded-t-3xl @md:rounded-3xl shadow-[var(--shadow-overlay)] p-5 pb-8 @md:pb-5 ${brand.bg} animate-[eu-sheet_.35s_var(--eu-ease-out)]`}>
        <div className="flex items-center justify-between mb-2">
          ...
          {stage === "review" && (
            <button type="button" onClick={onClose} aria-label="Ακύρωση" className={`size-9 rounded-full inline-flex items-center justify-center ${dark ? "bg-white/10" : "bg-eu-surface"}`}>
        ...
        {stage === "ok" ? (
          <div className="py-10 grid gap-3 justify-items-center text-center">
            ...
            <div className="font-extrabold text-[length:var(--fs-20)]">Έγινε</div>
            <div className={`text-[length:var(--fs-14)] ${dark ? "text-white/70" : "text-eu-muted"}`}>Η παραγγελία καταχωρείται…</div>
          </div>
        ) : stage === "processing" ? (
          <div className="py-10 grid gap-3 justify-items-center text-center">
            <Loader2 className="size-10 animate-spin" aria-hidden />
            <div className="font-bold text-[length:var(--fs-16)]">Επεξεργασία πληρωμής</div>
          </div>
        ) : stage === "auth" ? (
          <div className="py-10 grid gap-3 justify-items-center text-center">
          ...
          <>
            <div className={row}>
              ...
              <span className={`text-[length:var(--fs-13)] ${dark ? "text-white/60" : "text-eu-muted"}`}>Αλλαγή</span>
            </div>
            {address && (
              <div className={row}>
                ...
                <span className={`text-[length:var(--fs-13)] shrink-0 ${dark ? "text-white/60" : "text-eu-muted"}`}>Αλλαγή</span>
              </div>
            )}
            <div className={row}>
            ...
            <div className="flex items-center justify-between gap-3 py-3 text-[length:var(--fs-15)]">
              <span className="font-bold">ΠΛΗΡΩΜΗ ΣΕ EURONICS</span>
