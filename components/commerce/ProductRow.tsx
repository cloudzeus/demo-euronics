  ...
  return (
    <article className="bg-white rounded-lg shadow-[var(--shadow-card)] p-3.5 grid grid-cols-[96px_1fr] @md:grid-cols-[140px_1fr_220px] gap-4 items-start">
      <Link href={`/proion/${p.slug}`} className="relative aspect-square bg-eu-surface-2 rounded-md block">
        ...
        {pct !== null && <span className="absolute top-0 left-0 bg-eu-red text-white font-extrabold text-[length:var(--fs-10-5)] px-2 py-1 rounded-br-md rounded-tl-md">−{pct}%</span>}
      </Link>
      <div className="min-w-0">
        <div className="font-semibold text-eu-muted-2 text-[length:var(--fs-10)] tracking-wide">{p.brand}</div>
        <h3 className="m-0 font-bold text-eu-ink text-[length:var(--fs-14)] leading-[1.3]">
        ...
        {keySpecs.length > 0 && (
          <dl className="m-0 mt-2 grid grid-cols-2 gap-x-4 gap-y-0.5 text-[length:var(--fs-11-5)]">
            {keySpecs.map((s) => (
        ...
        <div className="@md:hidden mt-2 flex items-baseline gap-1.5">
          ...
          {p.wasPrice && <s className="text-eu-muted-2 text-[length:var(--fs-11)]">{priceShort(p.wasPrice)}</s>}
        </div>
      </div>
      <div className="col-span-2 @md:col-span-1 flex @md:flex-col gap-2 @md:gap-1.5 items-center @md:items-stretch">
        <div className="hidden @md:block">
          <div className="flex items-baseline gap-1.5">
            ...
            {p.wasPrice && <s className="text-eu-muted-2 text-[length:var(--fs-12)]">{priceShort(p.wasPrice)}</s>}
          </div>
          {p.lowest30 && <div className="text-eu-muted text-[length:var(--fs-10)] mt-1">Χαμηλότερη τιμή 30 ημερών: {priceLong(p.lowest30)}</div>}
          <div className="font-bold text-eu-blue text-[length:var(--fs-11-5)] mt-1 mb-2">ή 12 × {priceLong(instalment(p.price))} χωρίς κάρτα</div>
        </div>
        <button type="button" onClick={() => openQuickBuy(p)} className="flex-1 @md:flex-none rounded-full bg-eu-navy text-white font-extrabold text-[length:var(--fs-12-5)] py-3 min-h-11 hover:bg-eu-blue">
          Αγορά με 1 κλικ
        </button>
        <div className="flex gap-1.5">
          <button type="button" onClick={() => add(p)} className="flex-1 rounded-full border-2 border-eu-navy text-eu-navy font-extrabold text-[length:var(--fs-12)] py-2.5 min-h-11 hover:bg-eu-surface">
            Στο καλάθι
          </button>
