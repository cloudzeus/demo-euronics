      ...
      <SheetContent side="right" className="w-full sm:max-w-[420px] p-0 gap-0 border-0 shadow-[var(--shadow-overlay)]" showCloseButton={false}>
        <div className="flex flex-col h-full">
          <div className="flex justify-between items-center border-b border-eu-line-2 px-5 py-4">
            <SheetTitle className="font-extrabold text-eu-ink text-[length:var(--fs-16)]">
              {lastAdded ? "Προστέθηκε στο καλάθι" : "Καλάθι"} · {count}
            </SheetTitle>
            <button type="button" onClick={() => setMiniOpen(false)} className="inline-flex items-center gap-1 text-eu-muted-2 font-semibold text-[length:var(--fs-14)] min-h-11 px-2 hover:text-eu-ink">
              Κλείσιμο <X className="size-4" aria-hidden />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto px-5 py-3">
            {lines.length === 0 ? (
              <p className="text-eu-muted text-[length:var(--fs-15)] py-6 text-center m-0">Το καλάθι σου είναι άδειο.</p>
            ) : (
                ...
                {lines.map((l) => (
                  <li key={l.product.id + (l.variant ?? "")} className="flex gap-3 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {l.variant && <div className="text-eu-muted text-[length:var(--fs-13-5)]">{l.variant}</div>}
                      {l.addons.map((a) => (
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="inline-flex items-center border border-eu-line rounded-full">
                          ...
                          <span className="w-7 text-center font-bold text-[length:var(--fs-15)] tabular-nums">{l.qty}</span>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-eu-ink text-[length:var(--fs-16)]">{priceLong(l.qty * (l.product.price + l.addons.reduce((a, x) => a + x.price, 0)))}</span>
          ...
          {lines.length > 0 && (
            <div className="border-t border-eu-line-2 px-5 py-4 bg-eu-surface">
              ...
              <div className="flex justify-between font-extrabold text-eu-ink text-[length:var(--fs-16)] my-3">
                <span>Υποσύνολο</span>
                <span>{priceLong(subtotal + addonsTotal)}</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <Link href="/kalathi" onClick={() => setMiniOpen(false)} className="rounded-full border-2 border-eu-navy text-eu-navy text-center font-extrabold text-[length:var(--fs-15)] py-3 min-h-11 hover:bg-white">
                  Δες το καλάθι
                </Link>
                <Link href="/checkout" onClick={() => setMiniOpen(false)} className="rounded-full bg-eu-yellow text-eu-navy text-center font-extrabold text-[length:var(--fs-15)] py-3 min-h-11 hover:bg-eu-yellow-dark">
                  Ολοκλήρωση
                </Link>
              </div>
            </div>
