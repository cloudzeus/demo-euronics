...
export function AnnouncementBar({ left, right, accent, zoneNo }: Props) {
  return (
    <div className="relative bg-eu-navy text-eu-on-dark-2 eu-container">
      <ZoneBadge no={zoneNo} />
      <div className="eu-canvas eu-gutter flex justify-between items-center gap-4 py-2.5 font-semibold text-[length:var(--fs-11)] tracking-wide">
        ...
        <ul className="hidden @md:flex gap-5 m-0 p-0 list-none whitespace-nowrap">
          {right.map((t) => (
            <li key={t}>{t}</li>
          ))}
          {accent && (
