...
export function ServiceBooking({ service }: { service: { slug: string; title: string; priceFrom?: number } }) {
  ...
  if (done)
    return (
      <div className="bg-eu-green/10 border border-eu-green/30 rounded-xl p-4 text-[length:var(--fs-12-5)] text-eu-ink-2">
  ...
  return (
    ...
    >
      <div>
        <div className="font-extrabold text-eu-yellow text-[length:var(--fs-10-5)] tracking-wide">Κράτηση</div>
        <div className="font-bold text-[length:var(--fs-15)]">{service.title}</div>
        {service.priceFrom && <div className="text-eu-on-dark text-[length:var(--fs-12)]">από {priceLong(service.priceFrom)} · η τελική τιμή επιβεβαιώνεται τηλεφωνικά</div>}
      </div>
      ...
      <button type="submit" className="rounded-full bg-eu-yellow text-eu-navy font-extrabold text-[length:var(--fs-13)] py-3 min-h-11 hover:bg-eu-yellow-dark">
        Κλείσε ραντεβού
      </button>
    </form>
  );
