...
function SheetForm() {
  ...
  return (
    <div className="fixed inset-0 z-[70]" role="dialog" aria-modal="true" aria-labelledby="myspace-title">
      <button type="button" className="absolute inset-0 bg-eu-navy/50" aria-label="Κλείσιμο" onClick={() => setOpen(false)} />
      <form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="m-0 mt-1 text-eu-ink-3 text-[length:var(--fs-15)]">Δώσε τα μία φορά. Κάθε συσκευή θα σου λέει αν χωράει.</p>
          </div>
          <button type="button" onClick={() => setOpen(false)} aria-label="Κλείσιμο" className="size-11 rounded-full bg-eu-surface inline-flex items-center justify-center hover:bg-eu-surface-3">
        ...
        <label className="grid gap-1.5">
          ...
          <span className="text-eu-muted text-[length:var(--fs-14)]">Μέτρα το καθαρό άνοιγμα, όχι την κάσα.</span>
        </label>
        <label className="flex items-center justify-between gap-3 rounded-xl border-2 border-eu-line px-3 min-h-12 cursor-pointer">
          <span className="font-bold text-eu-ink text-[length:var(--fs-15)] inline-flex items-center gap-2">
            <ArrowUpDown className="size-4 text-eu-blue" aria-hidden /> Υπάρχει ασανσέρ
          </span>
          <input type="checkbox" checked={lift} onChange={(e) => setLift(e.target.checked)} className="size-5 accent-eu-blue" />
        </label>
        <label className="flex items-center justify-between gap-3 rounded-xl border-2 border-eu-line px-3 min-h-12 cursor-pointer">
          <span className="font-bold text-eu-ink text-[length:var(--fs-15)] inline-flex items-center gap-2">
            <Ruler className="size-4 text-eu-blue" aria-hidden /> Έχω συγκεκριμένη εσοχή
          </span>
