        ...
        <ul className="m-0 p-0 list-none flex @md:flex-col gap-2 order-2 @md:order-1 overflow-x-auto eu-scrollbar-none">
          {images.map((im, k) => (
            <li key={im} className="shrink-0">
              <button type="button" aria-label={`Εικόνα ${k + 1}`} aria-pressed={k === i} onClick={() => setI(k)} className={`relative size-16 rounded-md bg-eu-surface-2 border-2 ${k === i ? "border-eu-blue" : "border-transparent hover:border-eu-line"}`}>
                <Image src={im} alt="" fill sizes="64px" className="object-contain p-1" unoptimized={im.startsWith("http")} />
              </button>
            </li>
          ))}
        </ul>
