...
export function Reviews({ product: p }: { product: Product }) {
  ...
  return (
    <section className="mt-8" aria-labelledby="reviews" id="reviews">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        <h2 id="reviews" className="m-0 font-heading font-bold text-eu-ink text-[length:var(--fs-19)]">
          Αξιολογήσεις
        </h2>
        <div className="flex items-center gap-3">
          {count > 0 && (
            <span className="text-[length:var(--fs-12-5)] text-eu-muted">
          ...
          <button type="button" onClick={() => setOpen((o) => !o)} className="rounded-full border-2 border-eu-navy text-eu-navy font-extrabold text-[length:var(--fs-12)] px-4 min-h-10 hover:bg-eu-surface">
            Γράψε αξιολόγηση
          </button>
        </div>
      </div>
      {open && !sent && (
        ...
        >
          <div className="grid grid-cols-1 @sm:grid-cols-2 gap-3">
            <label className="grid gap-1 text-[length:var(--fs-12)] font-semibold">
              Όνομα
          ...
          <button type="submit" className="justify-self-start rounded-full bg-eu-navy text-white font-extrabold text-[length:var(--fs-12-5)] px-5 min-h-11 hover:bg-eu-blue">
            Υποβολή
          </button>
        </form>
      )}
      {sent && <p className="rounded-md bg-eu-chip text-eu-blue font-semibold text-[length:var(--fs-12-5)] px-3 py-2 mb-3">Ευχαριστούμε. Η αξιολόγησή σου θα δημοσιευθεί μετά από έλεγχο.</p>}
      {list.length === 0 ? (
        <p className="m-0 text-eu-muted text-[length:var(--fs-12-5)]">Δεν υπάρχουν ακόμη αξιολογήσεις για αυτό το προϊόν.</p>
      ) : (
          ...
          {list.map((r) => (
            <li key={r.id} className="rounded-lg border border-eu-line p-4">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span className="font-bold text-eu-ink text-[length:var(--fs-13)]">{r.title}</span>
                {r.verified && <span className="rounded-sm bg-eu-green/10 text-eu-green font-bold text-[length:var(--fs-10)] px-1.5 py-0.5">Επιβεβαιωμένη αγορά</span>}
              </div>
              <p className="m-0 text-eu-ink-2 text-[length:var(--fs-12-5)] leading-relaxed">{r.body}</p>
              <div className="text-eu-muted-2 text-[length:var(--fs-11)] mt-1">
                {r.author} · {r.date}
              </div>
            </li>
          ))}
