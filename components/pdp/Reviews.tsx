...
export function Reviews({ product: p }: { product: Product }) {
  ...
  return (
    <section className="scroll-mt-24" aria-labelledby="reviews-title" id="reviews">
      ...
      {sent && <p className="rounded-md bg-eu-chip text-eu-blue font-semibold text-[length:var(--fs-15)] px-3 py-2 mb-3">Ευχαριστούμε. Η αξιολόγησή σου θα δημοσιευθεί μετά από έλεγχο.</p>}
      {list.length === 0 ? (
        <p className="m-0 text-eu-muted text-[length:var(--fs-16)]">Δεν υπάρχουν ακόμη αξιολογήσεις για αυτό το προϊόν.</p>
      ) : (
          ...
          {list.map((r) => (
            <li key={r.id} className="rounded-lg border border-eu-line p-4">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                {r.verified && <span className="rounded-sm bg-eu-green/10 text-eu-green font-bold text-[length:var(--fs-13)] px-1.5 py-0.5">Επιβεβαιωμένη αγορά</span>}
              </div>
