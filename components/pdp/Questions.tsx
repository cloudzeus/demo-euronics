...
export function Questions({ product: p }: { product: Product }) {
  ...
  return (
    <section id="qa" className="scroll-mt-24" aria-labelledby="qa-title">
        ...
        <ul className="m-0 p-0 list-none grid gap-2 mb-4">
          {list.map((q) => (
            <li key={q.id} className="rounded-lg border border-eu-line p-4">
              ...
              {q.answer ? <p className="m-0 mt-1 text-eu-ink-2 text-[length:var(--fs-17)] leading-relaxed">Α: {q.answer}</p> : <p className="m-0 mt-1 text-eu-muted text-[length:var(--fs-14)]">Περιμένει απάντηση από την ομάδα μας.</p>}
      ...
      {sent ? (
        <p className="rounded-md bg-eu-chip text-eu-blue font-semibold text-[length:var(--fs-15)] px-3 py-2">Λάβαμε την ερώτησή σου. Απαντάμε εντός 1 εργάσιμης, με email.</p>
      ) : (
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <input id="q-input" name="q" required placeholder="Ρώτησε κάτι για το προϊόν" className="flex-1 min-w-0 rounded-full border border-eu-line px-4 py-2.5 min-h-11 text-[length:var(--fs-15)]" />
