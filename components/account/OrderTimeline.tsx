...
export function StatusChip({ status }: { status: Order["status"] }) {
  ...
  return <span className={`inline-block rounded-full px-2.5 py-1 font-bold text-[length:var(--fs-11)] ${s.tone}`}>{s.label}</span>;
}
    ...
    <div className="grid gap-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <div className="text-eu-muted text-[length:var(--fs-12)]">{new Date(o.date).toLocaleDateString("el-GR", { day: "numeric", month: "long", year: "numeric" })}</div>
        </div>
      ...
      <ol className="m-0 p-0 list-none grid grid-cols-4 gap-1">
        {steps.map((s, i) => (
          <li key={s} className="grid gap-1.5">
            ...
            <span className={`text-[length:var(--fs-10-5)] font-semibold ${i <= idx ? "text-eu-ink" : "text-eu-muted-2"}`}>{s}</span>
          </li>
        ))}
      </ol>
      {o.tracking && (
        <div className="rounded-lg bg-eu-surface p-4">
          <div className="flex flex-wrap justify-between gap-2 text-[length:var(--fs-12)] mb-2">
          ...
          <ul className="m-0 p-0 list-none grid gap-1.5 text-[length:var(--fs-12)]">
      ...
      {o.pickupStore && <p className="m-0 text-eu-ink-2 text-[length:var(--fs-12-5)]">Παραλαβή από: {o.pickupStore}</p>}
      <ul className="m-0 p-0 list-none divide-y divide-eu-line-2">
        {o.lines.map((l) => (
          <li key={l.productId} className="flex gap-3 py-2.5 items-center text-[length:var(--fs-12-5)]">
      ...
      <div className="flex justify-between font-extrabold text-eu-ink text-[length:var(--fs-15)] border-t border-eu-line pt-2">
