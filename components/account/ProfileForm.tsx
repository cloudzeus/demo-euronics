...
export function ProfileForm({ customer: c }: { customer: Customer }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      >
        <h2 className="m-0 font-heading font-bold text-eu-ink text-[length:var(--fs-20)]">Προσωπικά στοιχεία</h2>
        <div className="grid grid-cols-1 @md:grid-cols-2 gap-4">
          <label className={label}>
            Όνομα <input value={f.firstName} onChange={(e) => set("firstName", e.target.value)} className={input} autoComplete="given-name" />
          </label>
          <label className={label}>
            Επώνυμο <input value={f.lastName} onChange={(e) => set("lastName", e.target.value)} className={input} autoComplete="family-name" />
          </label>
          <label className={label}>
            Email <input type="email" value={f.email} onChange={(e) => set("email", e.target.value)} className={input} autoComplete="email" />
          </label>
          <label className={label}>
            Κινητό <input type="tel" value={f.phone} onChange={(e) => set("phone", e.target.value)} className={input} autoComplete="tel" />
          </label>
          <label className={label}>
            Ημερομηνία γέννησης <span className="font-normal text-eu-muted">(για δώρο γενεθλίων)</span>
            <input type="date" value={f.birthday} onChange={(e) => set("birthday", e.target.value)} className={input} />
          </label>
          <label className={label}>
            ΑΦΜ <span className="font-normal text-eu-muted">(για τιμολόγια)</span>
      ...
      <form
        ...
        className="bg-white rounded-2xl border border-eu-line p-5 @md:p-6 grid gap-4"
      >
        <h2 className="m-0 font-heading font-bold text-eu-ink text-[length:var(--fs-20)]">Ασφάλεια</h2>
        <div className="grid grid-cols-1 @md:grid-cols-3 gap-4">
          <label className={label}>
            Τρέχων κωδικός <input type="password" value={pw.cur} onChange={(e) => setPw({ ...pw, cur: e.target.value })} className={input} autoComplete="current-password" />
          </label>
          <label className={label}>
            Νέος κωδικός <input type="password" value={pw.next} onChange={(e) => setPw({ ...pw, next: e.target.value })} className={input} autoComplete="new-password" />
          </label>
          <label className={label}>
            Επανάληψη <input type="password" value={pw.again} onChange={(e) => setPw({ ...pw, again: e.target.value })} className={input} autoComplete="new-password" />
          </label>
        </div>
        <label className="flex items-start gap-3 rounded-xl bg-eu-surface p-4 cursor-pointer">
          ...
          <span className="text-[length:var(--fs-15)] text-eu-ink-2">
            <strong className="text-eu-ink inline-flex items-center gap-1.5">
              <ShieldCheck className="size-4 text-eu-green" aria-hidden /> Επαλήθευση δύο βημάτων
            </strong>
            <br />
      ...
      <section className="bg-white rounded-2xl border border-eu-line p-5 @md:p-6 grid gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => flash("Θα λάβεις email με το αρχείο εντός 24 ωρών.")} className="inline-flex items-center gap-2 rounded-full border-2 border-eu-navy text-eu-navy font-extrabold text-[length:var(--fs-15)] px-5 min-h-12 hover:bg-eu-surface">
            <Download className="size-4" aria-hidden /> Λήψη δεδομένων
          </button>
          <button type="button" onClick={() => flash("Η αίτηση διαγραφής καταχωρήθηκε. Θα επικοινωνήσουμε για επιβεβαίωση.")} className="inline-flex items-center gap-2 rounded-full border-2 border-eu-line text-eu-muted font-extrabold text-[length:var(--fs-15)] px-5 min-h-12 hover:border-eu-red hover:text-eu-red">
            <Trash2 className="size-4" aria-hidden /> Διαγραφή λογαριασμού
          </button>
        </div>
      </section>
    </div>
