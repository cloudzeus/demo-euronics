  ...
  return (
    <nav aria-label="Λογαριασμός" className="@lg:sticky @lg:top-4">
      <div className="hidden @lg:block bg-eu-surface rounded-xl p-4 mb-3">
        <div className="font-extrabold text-eu-ink text-[length:var(--fs-14)]">Γιάννης Παπαδόπουλος</div>
        <div className="text-eu-muted text-[length:var(--fs-11-5)]">giannis@example.gr</div>
      </div>
      <ul className="m-0 p-0 list-none flex @lg:flex-col gap-1 overflow-x-auto eu-scrollbar-none">
        {items.map((it) => {
          ...
          return (
            <li key={it.href} className="shrink-0">
              <Link href={it.href} aria-current={on ? "page" : undefined} className={`inline-flex @lg:flex items-center gap-2 rounded-full @lg:rounded-md px-3.5 py-2.5 min-h-11 font-semibold text-[length:var(--fs-12-5)] ${on ? "bg-eu-navy text-white" : "text-eu-ink-2 bg-eu-surface @lg:bg-transparent hover:bg-eu-chip"}`}>
        ...
        <li className="shrink-0">
          <button
            ...
            className="inline-flex @lg:flex items-center gap-2 rounded-full @lg:rounded-md px-3.5 py-2.5 min-h-11 font-semibold text-[length:var(--fs-12-5)] text-eu-muted hover:text-eu-red"
          >
