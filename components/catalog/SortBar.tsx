  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden @md:inline-flex rounded-full border border-eu-line overflow-hidden" role="group" aria-label="Προβολή">
          <button type="button" aria-pressed={view === "grid"} onClick={() => setParam("view", null)} className={`size-10 inline-flex items-center justify-center ${view === "grid" ? "bg-eu-navy text-white" : "text-eu-muted hover:bg-eu-surface"}`} aria-label="Πλέγμα">
            <LayoutGrid className="size-4" aria-hidden />
          </button>
          <button type="button" aria-pressed={view === "list"} onClick={() => setParam("view", "list")} className={`size-10 inline-flex items-center justify-center ${view === "list" ? "bg-eu-navy text-white" : "text-eu-muted hover:bg-eu-surface"}`} aria-label="Λίστα">
