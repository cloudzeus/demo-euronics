  ...
  return (
    <div className="bg-white rounded-lg p-4 flex flex-col shadow-[var(--shadow-card)]">
      <div className="flex justify-between items-center mb-3">
        <span className="font-extrabold text-eu-red text-[length:var(--fs-10-5)] tracking-wide">Προσφορά ημέρας</span>
      ...
      <div className="flex gap-3 items-center">
        ...
        <div className="min-w-0">
          <div className="font-medium text-eu-muted-2 text-[length:var(--fs-10)] mb-0.5">{p.brand}</div>
          <div className="font-bold text-eu-ink text-[length:var(--fs-12-5)] leading-[1.3]">{p.title}</div>
          <div className="flex items-baseline gap-1.5 mt-1.5 flex-wrap">
            <span className="font-extrabold text-eu-ink text-[length:var(--fs-19)] leading-none">{priceShort(p.price)}</span>
            {p.wasPrice && <s className="font-medium text-eu-muted-2 text-[length:var(--fs-11)]">{priceShort(p.wasPrice)}</s>}
            {pct !== null && <span className="bg-eu-red text-white font-extrabold text-[length:var(--fs-10)] px-1.5 py-0.5 rounded-sm">−{pct}%</span>}
          </div>
        </div>
      </div>
      <div className="text-eu-muted text-[length:var(--fs-10)] leading-snug my-2.5">
        {p.lowest30 && <>Χαμηλότερη τιμή 30 ημερών: {priceLong(p.lowest30)} · </>}ή 12 × {priceLong(instalment(p.price))} χωρίς κάρτα
      </div>
      <div className="flex gap-1.5 mt-auto">
        <button type="button" onClick={() => openQuickBuy(p)} className="flex-1 rounded-full bg-eu-navy text-white font-extrabold text-[length:var(--fs-12-5)] py-3 min-h-11 hover:bg-eu-blue">
          Αγορά με 1 κλικ
        </button>
