    ...
    <div className="eu-container">
      ...
      <div className="eu-canvas eu-gutter py-6">
        <form className="grid grid-cols-1 @md:grid-cols-[1fr_200px_200px_auto] gap-2 mb-5">
          <input name="q" defaultValue={sp.q} placeholder="Πόλη, Τ.Κ. ή όνομα καταστήματος" className="rounded-full border border-eu-line px-4 py-2.5 min-h-11 text-[length:var(--fs-13)]" aria-label="Αναζήτηση καταστήματος" />
          <select name="region" defaultValue={sp.region ?? ""} className="rounded-full border border-eu-line px-4 py-2.5 min-h-11 text-[length:var(--fs-13)] bg-white" aria-label="Νομός">
            <option value="">Όλοι οι νομοί</option>
            {regions.map((r) => (
              <option key={r}>{r}</option>
            ))}
          </select>
          <select name="service" defaultValue={sp.service ?? ""} className="rounded-full border border-eu-line px-4 py-2.5 min-h-11 text-[length:var(--fs-13)] bg-white" aria-label="Υπηρεσία">
        ...
        <div className="grid grid-cols-1 @lg:grid-cols-[minmax(0,1fr)_1fr] gap-5 items-start">
          <div>
            <div className="text-eu-muted text-[length:var(--fs-12-5)] mb-2">
            ...
            <ul className="m-0 p-0 list-none grid gap-3 max-h-[70vh] overflow-y-auto pr-1">
              {stores.map((s) => (
                <li key={s.id} className="bg-white rounded-xl border border-eu-line p-4 text-[length:var(--fs-12-5)]">
                  <div className="flex justify-between gap-2">
                    <Link href={`/katastimata/${s.slug}`} className="font-extrabold text-eu-ink text-[length:var(--fs-14)] hover:text-eu-blue">
                      {s.city} — {s.name}
                    </Link>
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {s.services.map((sv) => (
                      <span key={sv} className="rounded-full bg-eu-surface text-eu-ink-2 font-semibold text-[length:var(--fs-10-5)] px-2 py-0.5">
                        {SERVICE_LABEL[sv]}
                      </span>
                    ))}
                  </div>
                  <div className="flex gap-2 mt-3">
                    <Link href={`/katastimata/${s.slug}`} className="rounded-full bg-eu-navy text-white font-extrabold text-[length:var(--fs-11-5)] px-3.5 min-h-9 inline-flex items-center hover:bg-eu-blue">
                      Το κατάστημα
                    </Link>
                    <a href={`https://maps.google.com/?q=${s.lat},${s.lng}`} target="_blank" rel="noreferrer" className="rounded-full border-2 border-eu-navy text-eu-navy font-extrabold text-[length:var(--fs-11-5)] px-3.5 min-h-9 inline-flex items-center hover:bg-eu-surface">
                      Οδηγίες
                    </a>
                  </div>
                </li>
