    ...
    <div className="eu-container">
      ...
      <div className="eu-canvas eu-gutter pb-12 grid grid-cols-1 @lg:grid-cols-[minmax(0,1fr)_360px] gap-6 items-start">
        <div className="grid gap-6">
          {s.body && <p className="m-0 text-eu-ink-2 text-[length:var(--fs-16)] leading-relaxed">{s.body}</p>}
          {s.steps && (
            <section>
              ...
              <ol className="m-0 p-0 list-none grid gap-2">
                {s.steps.map((st, i) => (
                  <li key={st} className="flex gap-3 rounded-lg bg-eu-surface p-3 text-[length:var(--fs-15)] text-eu-ink-2">
                    <span className="size-7 shrink-0 rounded-full bg-eu-navy text-white font-extrabold inline-flex items-center justify-center text-[length:var(--fs-14)]">{i + 1}</span>
                    {st}
                  </li>
                ))}
              </ol>
              ...
              <div className="grid gap-2">
                {s.faq.map((f) => (
                  <details key={f.q} className="group rounded-lg border border-eu-line bg-white px-4">
                    ...
                    <p className="m-0 pb-3 text-eu-ink-2 text-[length:var(--fs-15)]">{f.a}</p>
                  </details>
                ))}
              </div>
            </section>
          ...
          {s.addonAt && (
            <section className="rounded-xl bg-eu-chip p-4 text-[length:var(--fs-15)] text-eu-ink-2">
        ...
        <aside className="grid gap-3 @lg:sticky @lg:top-4">
          ...
          <div className="bg-white rounded-xl border border-eu-line p-4">
            <h2 className="m-0 font-bold text-eu-ink text-[length:var(--fs-15)] mb-2">Άλλες υπηρεσίες</h2>
            <ul className="m-0 p-0 list-none grid gap-1 text-[length:var(--fs-15)]">
