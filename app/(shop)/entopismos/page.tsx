    ...
    <div className="eu-container">
      ...
      <div className="eu-canvas eu-gutter pb-12 grid grid-cols-1 @lg:grid-cols-[380px_minmax(0,1fr)] gap-6 items-start">
        <form action="/entopismos" className="bg-eu-surface rounded-xl p-5 grid gap-3">
          <label className="grid gap-1 text-[length:var(--fs-12)] font-semibold text-eu-ink">
            Αριθμός παραγγελίας
            <input name="no" defaultValue={no ?? ""} placeholder="EUR-20260904-0417" className="rounded-md border border-eu-line bg-white px-3 py-2.5 min-h-11 text-[length:var(--fs-13)]" />
          </label>
          <label className="grid gap-1 text-[length:var(--fs-12)] font-semibold text-eu-ink">
            Κινητό ή email
            <input name="contact" defaultValue={contact ?? ""} placeholder="69xxxxxxxx" className="rounded-md border border-eu-line bg-white px-3 py-2.5 min-h-11 text-[length:var(--fs-13)]" />
          </label>
          <button type="submit" className="rounded-full bg-eu-navy text-white font-extrabold text-[length:var(--fs-13)] py-3 min-h-11 hover:bg-eu-blue">
            Αναζήτηση
          </button>
          <p className="m-0 text-eu-muted text-[length:var(--fs-10-5)]">Δοκίμασε: EUR-20260904-0417 (σε διανομή), EUR-20260812-0093 (παραλήφθηκε).</p>
        </form>
        <div>
          {order ? (
            ...
            <div className="bg-white rounded-xl border border-eu-line p-5">
              ...
              <p className="m-0 mt-2 text-eu-ink-2 text-[length:var(--fs-13)]">Η παραγγελία καταχωρήθηκε μόλις τώρα και βρίσκεται σε επεξεργασία. Θα λάβεις SMS με τον αριθμό αποστολής μόλις παραδοθεί στον courier ή ετοιμαστεί στο κατάστημα.</p>
            </div>
          ) : no ? (
            <div className="bg-eu-surface rounded-xl p-5 text-eu-ink-2 text-[length:var(--fs-13)]">
